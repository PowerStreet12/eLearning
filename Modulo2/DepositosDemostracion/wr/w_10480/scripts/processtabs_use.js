...
processtabsUse1 = {
	...
	doUpdate: function() 
	{
		...
		if (tabCount == 2) {
			...
			tabWidthPercentage = 534 / tabCount;
		} else if (tabCount == 4) {
			tabWidthPercentage = 512 / tabCount;
		} else {
			tabWidthPercentage = 490 / tabCount;
		}
		...
		jQuery.each(textArray, function(index, value) {
			...
			button_elem.innerHTML += "<li><a style='width: " + tabWidthPercentage + "px' id='" + index + "' class='title'>" + textArray[index] + "</a></li>";			
			//add content first 
			...
			if (imageArray[index] == "-1") { 
				button_elem.innerHTML += "<div id='tab" + index + "' class='tab_content scroll-pane' style='display: none;'><p style='padding:10px 20px;'>" + buttonArray[index] + "</p></div>";				
		   } else {  
		   		if (picAlignArray[index] == 'LEFT') { newPad = 'padding-right:10px;' }
				else { newPad = 'padding-left:10px;' }
				button_elem.innerHTML += "<div id='tab" + index + "' class='tab_content scroll-pane' style='display: none;'><p style='padding:10px 20px;'><img style='" + newPad + "' width='150' align='" + picAlignArray[index] + "' height='150' src='" + imageArray[index] + "'/>" + buttonArray[index] + "</div> ";
		   }
 		 });	
